...
The ESP-IDF ADC unit, calibration, multisampling, and resource-cleanup primitives are available as a standalone module in [`src/adc/adc.c`](src/adc/adc.c) with its public interface in [`include/adc/adc.h`](include/adc/adc.h). The inverter-specific `adc_task()`, channel mapping, divider scaling, telemetry health, filtering, and safety policy are isolated in [`src/adc/adc_manager.c`](src/adc/adc_manager.c), declared by [`include/adc/adc_manager.h`](include/adc/adc_manager.h). See [`docs/adc_driver.md`](docs/adc_driver.md) for copy-and-integrate instructions for subsequent ESP32 projects.
